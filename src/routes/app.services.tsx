...
function ServicesPage() {
  ...
  const [form, setForm] = useState({ name: "", basePrice: 0, durationHrs: 1, warrantyDays: 30, description: "" });
  ...
  const submit = () => {
    ...
    setForm({ name: "", basePrice: 0, durationHrs: 1, warrantyDays: 30, description: "" });
    setOpen(false);
  };
  ...
  return (
    <div>
      <PageHeader eyebrow="Sector 06 · Catalog" title="Repair Services"
        action={<Btn variant="primary" onClick={() => setOpen(true)}><Plus className="inline w-4 h-4 mr-1" /> Add Service</Btn>}
      />
      ...
      <Modal open={open} onClose={() => setOpen(false)} title="Add Service">
        <div className="space-y-3">
          ...
          <div className="flex justify-end gap-2 pt-2"><Btn variant="ghost" onClick={() => setOpen(false)}>Cancel</Btn><Btn variant="primary" onClick={submit}>Save</Btn></div>
        </div>
      </Modal>
    </div>
  );
